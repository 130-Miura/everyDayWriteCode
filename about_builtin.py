...
# @staticmethodはメソッドを静的メソッドに変換する関数デコレータ。
# 静的メソッドは暗黙の第一引数を受け取らない。
# 静的メソッドはクラスからクラス.静的メソッド()、またはインスタンスからインスタンス().静的メソッド()などと呼び出せる
...
# tupple([iterable])は関数ではなくてイミュータブルなシーケンス型。

# type(object)
# type(name, bases, dict, **kwds)
# 引数が一つだけの場合、objectの型を返す。返り値はobject().__class__で返されるのと同じオブジェクト。
# オブジェクトの型判定にはisinstance()の利用が推奨される。理由は、サブクラスも考慮されるから。

# vars([object])はモジュール、クラス、インスタンスあるいはそれ以外の__dict__属性を持つオブジェクトの__dict__属性を返す。
# モジュールやインスタンスのようなオブジェクトは更新可能な__dict__属性を持っている。
# それ以外のオブジェクトでは__dict__属性への書き込みが制限されているものもある。
# 書き込み制限がある例として、types.MappingProxyTypeを使っているクラスがある。
